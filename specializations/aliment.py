...
alimenty_router = Router()
alimenty_router.message.middleware(AntiSpamMiddleware())
...
@alimenty_router.callback_query(F.data == "alimenty")
async def start_alimenty_test(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Start alimenty test error: {e}")
        await callback.answer("❌ Ошибка запуска теста")

@alimenty_router.message(StateFilter(TestStates.waiting_full_name))
...
@alimenty_router.message(StateFilter(TestStates.waiting_position))
...
@alimenty_router.message(StateFilter(TestStates.waiting_department))
async def process_department(message: Message, state: FSMContext):
    """Переход к выбору сложности."""
    try:
        data = await state.get_data()
        data["department"] = message.text.strip()
        data["specialization"] = "alimenty"
        await message.delete()
...
@alimenty_router.callback_query(F.data.startswith("diff_"))
async def select_difficulty(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Загрузка вопросов
        questions = load_questions_for_specialization("alimenty", difficulty, callback.from_user.id)
        if not questions:
...
@alimenty_router.callback_query(F.data.startswith("ans_"))
...
@alimenty_router.callback_query(F.data == "next_question")
