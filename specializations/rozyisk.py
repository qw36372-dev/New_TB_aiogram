"""
Роутер специализации "Исполнительный розыск и реализация имущества" — полный тест с FSM для продакшена.
...
async def select_difficulty(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await rozyisk_router.show_first_question(callback.message, test_state)
        await callback.answer()
...
async def toggle_answer(callback: CallbackQuery, state: FSMContext):
    ...
    await rozyisk_router.handle_answer_toggle(callback, TEST_STATES)

@rozyisk_router.callback_query(F.data == "next_question")
async def next_question(callback: CallbackQuery, state: FSMContext):
    """Переход к следующему вопросу. ✅ TestMixin"""
    await rozyisk_router.handle_next_question(callback, state, TEST_STATES)
...
async def handle_question_message(message: Message, state: FSMContext):
    ...
    await rozyisk_router.safe_start_question(message, state, TEST_STATES)
